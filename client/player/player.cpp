...
void Player::netThinkCallLord(int bet)
{
    grabLordBet(bet);
}

void Player::netThinkPlayHand(const Cards &cards)
{
    qDebug() << QThread::currentThreadId()<< "主线程中取出了Cards，准备发出信号++++++++++++++++++" ;
    playHand(cards);
    qDebug() << QThread::currentThreadId()<< "线程发完了信号++++++++++++++++++" ;
}
