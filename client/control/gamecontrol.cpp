...
{
    // 对象实例化
    m_robotLeft = new Robot("左机器人", this);
    m_robotRight = new Robot("右机器人", this);
...
Card GameControl::takeOneCard()
{
    ...
    if(flag)
    {
        Card c1(Card::Card_Q, Card::Club);
        Card c2(Card::Card_Q, Card::Diamond);
        Card c3(Card::Card_Q, Card::Heart);

        Card c4(Card::Card_K, Card::Club);
        Card c5(Card::Card_K, Card::Diamond);
        Card c6(Card::Card_K, Card::Heart);
        Card c7(Card::Card_K, Card::Spade);

        Card c8(Card::Card_A, Card::Spade);
        Card c9(Card::Card_A, Card::Diamond);
        Card c10(Card::Card_A, Card::Heart);
        Card c11(Card::Card_A, Card::Club);

        Card c12(Card::Card_BJ, Card::Suit_Begin);
        Card c13(Card::Card_SJ, Card::Suit_Begin);

        Card c14(Card::Card_2, Card::Club);
        Card c15(Card::Card_2, Card::Diamond);
        Card c16(Card::Card_2, Card::Heart);
        Card c17(Card::Card_2, Card::Spade);

        tmp << c1 << c2 << c3 << c4 << c5 << c6 << c7 << c8 << c9 << c10 << c11 << c12 << c13 << c14 << c15 << c16 << c17;
...
void GameControl::onGrabBet(Player *player, int bet)
{
    ...
    if(m_betRecord.times == 3)
    {
        if(m_betRecord.bet == 0)
        {
            ...
            {
                Message msg;
                msg.reqcode = RequestCode::ReDispatchCard;
    ...
    // 发送信号给主界面, 告知当前状态为抢地主
    emit playerStatusChanged(m_currPlayer, ThinkingForCallLord);
    qDebug() << "++++++++++下一位抢地主:" << m_currPlayer->getName();
    m_currPlayer->prepareCallLord();
}
