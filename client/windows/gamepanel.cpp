    ...
    , ui(new Ui::GamePanel)
{
    ...
    // // 计算居中位置（考虑物理像素和缩放比例）
    // qreal dpiScale = this->devicePixelRatio();


    // // 移动到屏幕中心
    // int x = (screenGeometry.width() - this->width() * dpiScale) / 2 / dpiScale;
    // int y = (screenGeometry.height() - this->height() * dpiScale) / 2 / dpiScale;
    // this->move(x, y);
    ...
    connect(comm, &Communication::reDispatchCard, this, [=](QByteArray msg){
       initGamePanel(msg);
    });
}
...
void GamePanel::initPlayerContext()
{
    ...
    {
        QPoint(rect().left()+6, cardsRect[0].height() / 2 + 20),     // 左侧机器人
        QPoint(rect().right()-90, cardsRect[1].height() / 2 + 20),    // 右侧机器人
        QPoint(cardsRect[2].right()-15, cardsRect[2].top() - 10)            // 当前玩家
    };
...
{
    // 播放背景音乐
    BGMControl::getInstance()->playAssistMusic(BGMControl::Dispatch);
    // 重置每张卡牌的属性
...
//        disposeCard(curPlayer, cs);
        ...
        if(m_gameCtl->getSurplusCards().cardCount() == 3)
        {
            ...
            // 终止发牌音乐的播放
            BGMControl::getInstance()->stopAssistMusic();
            return;
        }
    }
    // 移动扑克牌
...
void GamePanel::onPlayerStatusChanged(Player *player, GameControl::PlayerStatus status)
{
    ...
    {
        BGMControl::getInstance()->stopBGM();
...
void GamePanel::onGrabLordBet(Player *player, int bet, bool flag)
{
    ...
    // 播放分数的背景音乐
    BGMControl::getInstance()->playerRobLordMusic(bet, (BGMControl::RoleSex)player->getSex(), flag, m_playerList.indexOf(player));
}
...
void GamePanel::onDisposePlayHand(Player *player, const Cards &cards)
{
    ...
    if(cards.isEmpty())
    {
        ...
        it->info->show();
        BGMControl::getInstance()->playPassMusic((BGMControl::RoleSex)player->getSex(), m_playerList.indexOf(player));
    }
    else
    {
        if(m_gameCtl->getPendPlayer() == player || m_gameCtl->getPendPlayer() == nullptr)
        {
            BGMControl::getInstance()->playCardMusic(cards, true, (BGMControl::RoleSex)player->getSex(), m_playerList.indexOf(player));
        }
        else
        {
            BGMControl::getInstance()->playCardMusic(cards, false, (BGMControl::RoleSex)player->getSex(), m_playerList.indexOf(player));
        }
    }
    // 3. 更新玩家剩余的牌
    updatePlayerCards(player);
    ...
    {
        BGMControl::getInstance()->playLastMusic(BGMControl::Last2, (BGMControl::RoleSex)player->getSex(), m_playerList.indexOf(player));
    }
    else if(player->getCards().cardCount() == 1)
    {
        BGMControl::getInstance()->playLastMusic(BGMControl::Last1, (BGMControl::RoleSex)player->getSex(), m_playerList.indexOf(player));
    }
}
...
void GamePanel::onCardSelected(Qt::MouseButton button)
{
    ...
    if(button == Qt::LeftButton)
    {
        ...
        BGMControl::getInstance()->playAssistMusic(BGMControl::SelectCard);
    }
...
void GamePanel::showEndingScorePanel()
{
    ...
    if(isWin)
    {
        BGMControl::getInstance()->playEndingMusic(true);
    }
    else
    {
        BGMControl::getInstance()->playEndingMusic(false);
    }
    ...
    connect(panel, &EndingPanel::continueGame, this, [=]()
    {
        ...
        if(DataManager::getInstance()->isNetworkMode())
        {
            ...
            msg.userName = DataManager::getInstance()->getUserName();
            DataManager::getInstance()->getCommunication()->sendMessage(&msg);
            DataManager::getInstance()->setIsRestart(true);
            this->close();
        }
        else
        {
            BGMControl::getInstance()->startBGM(80);
...
void GamePanel::initCountDown()
{
    ...
    connect(m_countDown, &CountDown::notMuchTime, this, [=](){
        // 播放提示音乐
        BGMControl::getInstance()->playAssistMusic(BGMControl::Alert);
    });
    ...
    connect(m_countDown, &CountDown::timeout, this,[=](){
        if( m_gameCtl->getPendPlayer() != nullptr && m_gameCtl->getPendPlayer() != m_gameCtl->getUserPlayer())
        {
            ...
            if(curPlayer == userPlayer)
            {
                CardList list =userPlayer->getCards().toCardList();
                CardPanel* cpanel = m_cardMap[list.last()];
                emit cpanel->cardSelected(Qt::LeftButton);
                emit ui->btnGroup->playHand();
            }
        }
...
void GamePanel::initGamePanel(QByteArray msg)
{
    ...
    {
        auto sub = item.split('|');
        order.insert(sub.at(1).toInt(),  QPair<QByteArray, int>(sub.at(0), sub.at(2).toInt()));
...
{
    BGMControl::getInstance()->startBGM(80);
    CardsQueue::getInstance()->clear();
    BetQueue::getInstance()->clear();
    // 设置当前被发牌的玩家
...
{
    BGMControl::getInstance()->stopMusic();
    emit panelClose();
    // 判断游戏模式
    if(DataManager::getInstance()->isNetworkMode())
    {
        CardsQueue::getInstance()->release();
        BetQueue::getInstance()->release();
        if (!DataManager::getInstance()->isRestart())
        {
            Message msg;
        ...
        {
            DataManager::getInstance()->setIsRestart(false);
        }
    }
