    ...
    , ui(new Ui::JoinRoom)
{
    ...
    connect(comm, &Communication::roomExist, this, [=](bool flag){
        ...
        {
            QMessageBox::critical(this, "搜索房间", "找不到房间或者房间号无效");
            ui->joinRoom->setBtnDisable(true);
        }
    });
    connect(comm, &Communication::joinRoomFailed, this, [=](QByteArray msg){
        show();
        QMessageBox::critical(this, "创建房间", msg);
    });

    connect(ui->createRoom, &QPushButton::clicked, this,  [=](){
        emit leaveRoom();
        joinRoom();
    });
