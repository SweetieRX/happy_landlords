...
bool PlayHand::canBeat(const PlayHand &other)
{
    ...
    if(m_type == Hand_Bomb && other.m_type >= Hand_Single && other.m_type <=  Hand_Bomb_Jokers_Two_Single)
    {
        return true;
    }
    // 双方的牌型一致
...
{
    if(m_oneCard.size() == 2 && m_twoCard.size() == 2 && m_threeCard.isEmpty() && m_fourCard.isEmpty())
    {
        std::sort(m_oneCard.begin(), m_oneCard.end());
